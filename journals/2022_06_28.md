...
- # 变量命名与注释规范 #[[编码规范]]
	- 命名：
	  1. 生命周期短的变量简短命名。比如 for 循环中的索引会有 i、j 来代替。
	  2. 生命周期长的变量，变量名能够表示他的作用。比如 expireItems 就能表示过期的一些事项。
	  3. 变量名如果有一些跟上下文重合的信息，应该舍去。比如：AddUser(username, userpassword)，其中变量中的 user 都可以去掉。